...
export default function Auth() {
  ...
  const { isBlocked, isChecking, blockReason } = useBlockCheck();
  ...
  useEffect(() => {
    if (rateLimitTime > 0) {
      const timer = setInterval(() => {
        setRateLimitTime(prev => prev > 0 ? prev - 1 : 0);
      }, 1000);
      return () => clearInterval(timer);
    }
  }, [rateLimitTime]);

  // Save device info after successful login
  const saveDeviceInfo = async (userId: string) => {
    if (!deviceInfo?.fingerprint) return;
    
    try {
      await supabase.from('user_device_info').upsert({
        user_id: userId,
      ...
      <div className="min-h-screen bg-background flex items-center justify-center p-6">
        <div className="max-w-md w-full text-center space-y-6">
          ...
          <div className="space-y-2">
            <h1 className="text-2xl font-bold text-red-500">Thiết bị bị chặn</h1>
    ...
    return (
      <div className="min-h-screen bg-background flex items-center justify-center p-6">
        <div className="max-w-md w-full text-center space-y-6">
          ...
          <div className="space-y-2">
            <h1 className="text-2xl font-bold text-red-500">Tài khoản đã bị xóa</h1>
            <p className="text-muted-foreground">
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (isRateLimited(rateLimitKey)) {
      const remainingTime = getRateLimitRemainingTime(rateLimitKey);
      setRateLimitTime(remainingTime);
      toast({
        title: 'Quá nhiều lần thử',
        description: `Vui lòng đợi ${Math.ceil(remainingTime / 60)} phút trước khi thử lại`,
        variant: 'destructive'
      });
      return;
    }
    ...
    try {
      if (isLogin) {
        ...
        if (error) {
          // Check if account was deleted (user not found)
          if (error.message.includes('Invalid login credentials')) {
            // Could be deleted account or wrong password
            // We'll show a generic error first
            throw error;
          }
          throw error;
        }
        ...
        if (loggedInUser) {
          await saveDeviceInfo(loggedInUser.id);
        }
        ...
        if (newUser) {
          await saveDeviceInfo(newUser.id);
        }
      ...
      // Check for specific error messages indicating deleted account
      if (error.message?.includes('User not found') || 
          (error.message?.includes('Invalid login credentials') && isLogin)) {
        // Could be a deleted account
        toast({
          title: 'Đăng nhập thất bại',
          description: 'Email hoặc mật khẩu không đúng. Nếu tài khoản đã bị xóa, bạn không thể đăng nhập.',
          variant: 'destructive'
        });
      } else {
        toast({
